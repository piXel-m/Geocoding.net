...
namespace Geocoding.MapQuest
{
	...
	public class MapQuestGeocoder : IGeocoder, IBatchGeocoder
	{
		...
		private HttpWebRequest Send(BaseRequest f)
		{
			if (f == null)
			...
			if (hasBody)
			{
				...
				using (Stream rs = request.GetRequestStreamAsync().Result)
				{
		...
		private MapQuestResponse Parse(HttpWebRequest request)
		{
			if (request == null)
			...
			try
			{
				...
				using (HttpWebResponse response = request.GetResponseAsync().Result as HttpWebResponse)
				{
					if ((int)response.StatusCode >= 300) //error
						throw new HttpException(response.StatusDescription, (int)response.StatusCode);

					using (var sr = new StreamReader(response.GetResponseStream()))
						json = sr.ReadToEnd();
				}
				if (string.IsNullOrWhiteSpace(json))
					throw new FormatException("Remote system response with blank: " + requestInfo);

				MapQuestResponse o = json.FromJSON<MapQuestResponse>();
				if (o == null)
					throw new FormatException("Unable to deserialize remote response: " + requestInfo + " => " + json);
			...
			catch (WebException wex) //convert to simple exception & close the response stream
			{
				using (HttpWebResponse response = wex.Response as HttpWebResponse)
				{
					...
					throw new HttpException(sb.ToString(), (int)response.StatusCode);
				}
			}
		}
